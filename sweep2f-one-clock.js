/**
 * SWEEP 2F - ONE CLOCK (slope clause deleted; floor = 24 NQ-clock hours)
 *
...
(function main() {
  ...
  for (let g = 0; g < grid.length; g++) {
    ...
    if (rank >= 0.95 && fr > 0) s = 'SHORT';
      ...
      if (sawHi && fr > 0 && fr < hiFr && smokeRank3h(longM, t) >= 0.80) s = 'SHORT';
      else if (sawLo && fr < 0 && fr > loFr && smokeRank3h(shortM, t) >= 0.80) s = 'LONG';
    }
  ...
  const VARIANTS = [
    { name: 'strict ep15/P90-90m/P60-24h', ep: 15, st: 90, sw: 90, fl: 60, lw: 1440 },
    { name: 'loose  ep10/P85-60m/P50-24h', ep: 10, st: 85, sw: 60, fl: 50, lw: 1440 }
  ];

  const out = [];
  out.push('SWEEP 2F - ONE CLOCK  ' + new Date().toISOString());
  out.push('');
  ...
  for (const V of VARIANTS) {
    function magRank(t, mins) {
      ...
      while (n < target && guard++ < 4000) {
        m -= 5 * 60000;
        if (isWeekend(m)) continue;               // NQ clock: weekend costs nothing
        if (Math.abs(epSums(m, V.ep).dOI) < cur) below++;
        n++;
      }
      return n ? below / n : 0;
    }
    ...
    let open = null;   // {ms, dir, px} - live position for stop tracking
    for (const t of msArr) {
      if (t < start) continue;
      if (open) {
        const pp = byMin.get(t);
        if (pp && pp.px) {
          const adv = open.dir === 'SHORT' ? (pp.px - open.px) / open.px : (open.px - pp.px) / open.px;
          if (adv >= 0.0075) { open = null; runFired = false; }   // STOPPED -> run re-arms
        }
      }
      if (isWeekend(t)) continue;   // CME dark: no entries
      const qh = new Date(t + 36e6).getUTCHours();
      if (qh >= 5 && qh < 8) continue;   // entry blackout: 05:00-08:00 Qld (close gap)
      ...
      disarmSince = null;
      if (dir !== runDir) { runDir = dir; runFired = false; open = null; }
      ...
      if (magRank(t, V.sw) >= V.st / 100 && magRank(t, V.lw) >= V.fl / 100) {
        if (!runFired) { entries.push({ ms: t, dir }); runFired = true; const pE = priceAt(t); if (pE) open = { ms: t, dir, px: pE }; }
    ...
    for (const en of entries) {
      ...
      // exit horizon: fixed 12h, cut only by the next OPPOSITE-side entry
      let horizon = en.ms + 12 * 3600e3;
      for (const other of entries) {
        if (other.ms > en.ms && other.dir !== en.dir) { horizon = Math.min(horizon, other.ms); break; }
      }
  ...
  fs.writeFileSync('sweep2f_results.txt', out.join('\n') + '\n');
  console.log('wrote sweep2f_results.txt');
})();
