...
(function main() {
  ...
  function ep(t) {  // trailing 15m sums
    let dOIc = 0, dOIu = 0, delta = 0, liq = 0;
    for (let m = t - 14 * 60000; m <= t; m += 60000) {
      const p = byMin.get(m);
      if (p) { dOIc += p.dOIc; dOIu += p.dOIu; delta += p.delta; }
      liq += liqM.get(m) || 0;
    }
    return { dOIc, dOIu, delta, liq };
  }
  function frNear(t) {
  ...
  function magRank(t, mins) {
    ...
    let below = 0, n = 0;
    for (let m = t - mins * 60000; m < t; m += 5 * 60000) { if (Math.abs(ep(m).dOIc) < cur) below++; n++; }
    return n ? below / n : 0;
  }
  function gapRank(t) {
  ...
  const out = [];
  out.push('SWEEP 3 - HARVEST BELL  ' + new Date().toISOString());
  out.push('');
  ...
  for (const F of [1.0, 1.5]) for (const G of [90, 95]) {
    ...
    for (const t of msArr) {
      ...
      const authored = Math.abs(e.dOIu) / (e.liq + Math.abs(e.delta) + 1);
      if (authored < F) continue;                                  // 2: authored
  ...
  fs.writeFileSync('sweep3_results.txt', out.join('\n') + '\n');
  console.log('wrote sweep3_results.txt');
})();
