/**
 * SWEEP 2D - STOP-AND-REARM (0.75% stop frees the run for re-entry)
 *
...
(function main() {
  ...
  const out = [];
  out.push('SWEEP 2D - STOP-AND-REARM  ' + new Date().toISOString());
  out.push('');
  ...
  fs.writeFileSync('sweep2d_results.txt', out.join('\n') + '\n');
  console.log('wrote sweep2d_results.txt');
})();
