...
function rotateLogs() {
    ...
    ensureCSV(PILLAR_LOG, 'qld_time,timestamp,eth_price,nq_price,yellow_count,funding_open,funding_high,funding_low,funding_close,oi_open,oi_high,oi_low,oi_close,cvd_agg_buy,cvd_agg_sell,cvd_delta,oi_coin_open,oi_coin_high,oi_coin_low,oi_coin_close\n');
...
async function pillarSnapshot(wethPrice, nqP) {
    ...
    const [pillars, deribit, cvd, optMaxPain, liqOrders, fundOHLC, oiOHLC, oiCoinOHLC] = await Promise.all([
        getFourPillars(),
    ...
    rotateLogs();
    const row = qldStr + "," + nowIso + "," + wethPrice.toFixed(2) + "," + (nqP > 0 ? nqP.toFixed(2) : "") + "," + activeYellowCount + ","
